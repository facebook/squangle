...
void numericDynamic(int numericCount) {
  for (int i = 0; i < numericCount; ++i) {
    folly::dynamic v(0);
    v = int(1); v = double(2.0); v = short(3);
    v = int(1); v = double(2.0); v = short(3);
    v = int(1); v = double(2.0); v = short(3);
    v = int(1); v = double(2.0); v = short(3);
    v = int(1); v = double(2.0); v = short(3);
...
void stringDynamic(int count) {
  folly::fbstring fb;
  BENCHMARK_SUSPEND { fb = folly::fbstring(fb); }
...
BENCHMARK(createQueryObject) {
  for (int i = 0; i < kCount; ++i) {
    ...
    QUERY(("SELECT * FROM table1 WHERE column3 = %d AND column5 = %s",
           14,
           "table1"));
    QUERY(("SELECT * FROM table1 WHERE %C = %f", "column1", 2.2));
    QUERY(("SELECT * FROM table1 WHERE %W",
    ...
    QUERY(("SELECT COUNT(*) FROM table1 WHERE %LA",
           testObject("col1", "red")("col2", "blue")));
    QUERY(("SELECT COUNT(*) FROM table1 WHERE %LO",
           testObject("col1", "red")("col2", "blue")));
    QUERY(("SELECT COUNT(*) FROM table1 WHERE %LA",
           testObject("col1", "red")("col2", nullptr)));
    QUERY(("SELECT COUNT(*) FROM table1 WHERE %LA AND %LO",
           testObject("color", "yellow")("species", "cat"),
           testObject("color", "brown")("species", "dog")));
    QUERY(("UPDATE %T SET %U WHERE 1",
           "table2",
           testObject("column1", "red")("column2", "small")));
    QUERY(("UPDATE %T SET col1 = value1 WHERE %W",
           "table2",
...

    {
      ...
      QUERY(("INSERT INTO %T (%C, %C, %C) VALUES %V",
             "table1",
             "a",
             "b",
             "c",
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < keys_count; ++i) {
    keys.push_back("this is a key and we are having fun " +
